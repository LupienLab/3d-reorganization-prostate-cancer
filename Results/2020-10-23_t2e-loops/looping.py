...
Identify loops of regions to test for that contain differential acetylation, looping, and expression
"""
...
def compatible_sv_pair(
    a: GenomicInterval, b: GenomicInterval, a_type: str, b_type: str
) -> bool:
    """
    ...
    """
    position = False
    if overlapping(a, b, TOL // 2) or equivalent_tad(
        a, b, tads[a.data["sample"]], tads[b.data["sample"]]
    ):
        position = True
...
def compatible_svs(
    a: GenomicInterval,
    b: GenomicInterval,
    a_nbrs: nx.classes.coreviews.AtlasView,
    b_nbrs: nx.classes.coreviews.AtlasView,
) -> Tuple[bool, Set[str]]:
    """
    ...
    for an in a_nbrs.keys():
        ...
        for bn in b_nbrs.keys():
            if compatible_sv_pair(
                an, bn, a_nbrs[an]["annotation"], b_nbrs[bn]["annotation"]
            ):
                compatible_nbr = True
                break
...
SAMPLES = CONFIG.loc[CONFIG.Include == "Yes", "Sample ID"].tolist()

# load loops
difftl_loops = pd.concat(
    [
        pd.read_csv(
...
difftl_loops.reset_index(inplace=True)
difftl_loops.drop(labels=["level_1"], axis=1, inplace=True)
difftl_loops.rename({"level_0": "loop_type"}, axis=1, inplace=True)

# acetylation and anchor intersections
acetyl_anchors = pd.read_csv(
...
# add each overlapping anchor as a node to the graph
for dl in difftl_loops.itertuples():
    ...
    for intvl in intvls:
        G.add_node(intvl)
    # link the two nodes togethers
    G.add_edge(intvls[0], intvls[1])

# link anchors together if they make a loop together
for dl in difftl_loops.itertuples():
    anchor_x = [
        n for n in G if n.data["type"] == "anchor" and n.data["id"] == dl.anchor_ID_x
    ]
    anchor_y = [
        n for n in G if n.data["type"] == "anchor" and n.data["id"] == dl.anchor_ID_y
    ]
    if len(anchor_x) > 1:
        print(anchor_x)
    if len(anchor_y) > 1:
...
G_sample_copy, confirmed_bp_sets_to_merge_copy = pickle.load(
    open(path.join(GRAPH_DIR, "breakpoints.per-sample.with-multiplicity.p"), "rb")
)
...
pickle.dump(
    G_sample,
    open(path.join(GRAPH_DIR, "breakpoints.per-sample.merged-breakpoints.p"), "wb"),
)

# create compiled list of all breakpoints for easier parsing and saving in a table
uncoupled_breakpoints = pd.DataFrame(
    columns=[
        "chr",
        "start",
        "end",
        ...
        if equivalent_tad(n, m, tads[n.data["sample"]], tads[m.data["sample"]]):
            ...
            if is_compatible:
                ...
                if n.data["test_ID"] in test_ID_SV_types:
                    test_ID_SV_types[n.data["test_ID"]] = test_ID_SV_types[
                        n.data["test_ID"]
                    ].union(compatible_types)
                else:
                ...
                if not m_previously_assigned_test:
                    ...
                    test_ID_SV_types[j] = set(
                        [e["annotation"] for e in G_all[m].values()]
                    )
...
for cc in nx.connected_components(ttm_graph):
    ...
    for n in cc:
        ...
        if n in test_ID_SV_types:
            test_ID_SV_types[min_test_ID] = test_ID_SV_types[min_test_ID].union(
                test_ID_SV_types[n]
            )
...
for n in G_all:
    if n.data["test_ID"] in ttm_new_IDs:
        ...
        test_ID_status[ttm_new_IDs[n.data["test_ID"]]] = (
            test_ID_status[ttm_new_IDs[n.data["test_ID"]]]
            and test_ID_status[n.data["test_ID"]]
        )
...
for t_id in test_ID_status:
    ...
    for e in ttnm_graph.edges():
        ...
        if t_id == e[0]:
            incompatible_samples = set(
                [n.data["sample"] for n in G_all if n.data["test_ID"] == e[1]]
            )
        elif t_id == e[1]:
            incompatible_samples = set(
                [n.data["sample"] for n in G_all if n.data["test_ID"] == e[0]]
            )
    ...
    uncoupled_breakpoints.loc[
        uncoupled_breakpoints.breakpoint_ID.isin(b_ids), "test_ID"
    ] = int(t_id)
    paired_breakpoints.loc[
        paired_breakpoints.breakpoint_ID_x.isin(b_ids), "test_ID_x"
    ] = t_id
    paired_breakpoints.loc[
        paired_breakpoints.breakpoint_ID_y.isin(b_ids), "test_ID_y"
    ] = t_id

# save coupled tests and breakpoints
uncoupled_breakpoints.sort_values(by=["SampleID", "chr", "start", "end"]).to_csv(
    path.join(GRAPH_DIR, "sv-breakpoints.tsv"), sep="\t", index=False
)

paired_breakpoints.sort_values(
    by=["SampleID", "chr_x", "start_x", "end_x", "chr_y", "start_y", "end_y"]
).to_csv(path.join(GRAPH_DIR, "sv-breakpoints.paired.tsv"), sep="\t", index=False)

coupled_tests.to_csv(
    path.join(GRAPH_DIR, "sv-disruption-tests.tsv"), sep="\t", index=False
)
...
for s in SAMPLES:
    component_counts.at[component_counts.SampleID == s, "Events"] = len(
        [cc for cc in nx.connected_components(G_sample[s])]
    )
    component_counts.at[component_counts.SampleID == s, "Complex_Events"] = len(
        [cc for cc in nx.connected_components(G_sample[s]) if len(cc) > 2]
    )
    component_counts.at[
        component_counts.SampleID == s, "Smallest_Component_Size"
    ] = np.min([len(cc) for cc in nx.connected_components(G_sample[s])])
    component_counts.at[
        component_counts.SampleID == s, "Largest_Component_Size"
    ] = np.max([len(cc) for cc in nx.connected_components(G_sample[s])])
    component_counts.at[
        component_counts.SampleID == s, "Smallest_Component_ID"
    ] = np.argmin([len(cc) for cc in nx.connected_components(G_sample[s])])
    component_counts.at[
        component_counts.SampleID == s, "Largest_Component_ID"
    ] = np.argmax([len(cc) for cc in nx.connected_components(G_sample[s])])
...
component_counts.to_csv(
    path.join("Statistics", "breakpoint-components.tsv"), sep="\t", index=False
)

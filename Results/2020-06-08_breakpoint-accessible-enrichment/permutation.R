...
# load H3K27ac peaks
peaks <- fread(
    paste0("../../Data/External/TCGA/PRAD_peakCalls.txt"),
    sep = "\t",
    header = FALSE,
)
...
# calculate how much of each SV breakpoint is accessible chromatin
pct_accessible <- unlist(lapply(
    1:length(bp_gr),
    function(i) {
        common <- commonRegions(bp_gr[i], peaks)
        # calculate percent of breakpoint that overlaps an ATAC-seq peak
        return(sum(width(common)) / width(bp_gr[i]))
    }
))
bp_gr$accessibility <- pct_accessible
breakpoints[, Accessibility := pct_accessible]
...
gg <- (
    ...
    + labs(x = "Breakpoints", y = "Overlapping Accessible Chromatin Peaks")
    + scale_fill_manual(
...
fwrite(
    breakpoints,
    "Overlaps/sv-breakpoints.accessibility.tsv",
    sep = "\t",
    col.names = TRUE
)

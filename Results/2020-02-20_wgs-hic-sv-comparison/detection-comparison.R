...
hic <- fread(
    file.path("..", "2020-02-19_chromoplexy", "Graphs", "sv-breakpoints.tsv"),
    header = TRUE,
    sep = "\t"
)
hic_gr <- dt2gr(hic)
...
hic_pairs <- fread(
    file.path("..", "2020-02-19_chromoplexy", "Graphs", "sv-breakpoints.paired.tsv"),
    header = TRUE,
    sep = "\t"
)
...
htests <- list(
    ...
    Accessibility_Extended = mantelhaen.test(accessibility_extended_array, alternative = "less"),

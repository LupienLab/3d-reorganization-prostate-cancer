...
wgs = pd.concat(
    [
        pd.read_csv(
            path.join("..", "..", "Data", "External", "CPC-GENE", "structural-variant-vcfs", s + ".hg38.sorted.bed"),
            sep="\t",
            header=None,
...
hic = pd.read_csv(
    path.join("..", "2020-02-19_chromoplexy", "Graphs", "sv-breakpoints.tsv"),
    sep="\t",
    header=0,
    index_col=False,
)
...
for s in SAMPLES:
    ...
    # for each breakpoint called from the WGS data, try to identify a similar breakpoint(s) from the Hi-C data
    for r in sample_wgs.itertuples():
        similar_hic_breaks = sample_hic.loc[(sample_hic.chr == r.chr) & (r.start - TOL <= sample_hic.end) & (r.end + TOL >= sample_hic.start),:]
        if similar_hic_breaks.shape[0] > 0:
            wgs.loc[r.Index, "Mutual"] = True
            wgs.loc[r.Index, "Mutual_Breakpoint_IDs"] = ",".join([str(i) for i in similar_hic_breaks.breakpoint_ID])
    # for each breakpoint called from the Hi-C data, try to identify a similar breakpoint(s) from the WGS data

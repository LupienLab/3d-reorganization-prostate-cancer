...
if (!interactive()) {
    PARSER <- argparse::ArgumentParser(
        description = "Convert a sparse COO-based matrix file to a wide, dense matrix file"
    )
    PARSER$add_argument(
        "data",
        type = "character",
    ...
    ARGS = list(
        data = "TMP/PCa13266.4000bp.chr2.annotated.mtx",
        prefix = "TMP/PCa13266"
    )
}
...
write.table(
    dmat,
    file=paste0(ARGS$prefix, ".wide.mtx"),
    sep = "\t",
    col.names = FALSE,
    row.names = FALSE,
    # replace NAs with 0 (not doing so will throw errors in TADsplimer)
    na = "0"
)

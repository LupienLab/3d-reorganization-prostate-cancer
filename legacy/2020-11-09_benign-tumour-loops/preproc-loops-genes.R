...
loops <- rbindlist(lapply(
    c("tumour-specific", "benign-specific", "shared"),
    function(loop_type) {
        dt <- fread(
...
# repeat reduction process until no changes to loops and genes occurs
counter <- 0
while (TRUE) {
    counter <- counter + 1
    print(counter)
    reduction <- reduce_elements(prom_gr, anch_gr)
    reduced_prom <- reduction$prom
    reduced_anch <- reduction$anch
    if (all(identical(reduced_prom, prom_gr), identical(reduced_anch, anch_gr))) {
        break
    } else {

...
fwrite(
    perm_test_data,
    "Overlaps/all-dmrs.permutations.tsv",
    sep = "\t",
    col.names = TRUE
)
...
fwrite(
    lz_dt,
    "Overlaps/all-dmrs.local-dependency.tsv",
    sep = "\t",
    col.names = TRUE
)

saveRDS(perm_test, "Overlaps/all-dmrs.permutation-tests.rds")
saveRDS(lz, "Overlaps/all-dmrs.local-dependency.rds")

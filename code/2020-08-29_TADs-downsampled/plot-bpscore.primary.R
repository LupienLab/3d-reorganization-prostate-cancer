...
PLOT_DIR <- file.path("Plots", "primary")
...
htests <- list(
    "primary_benign-only_benign-vs-tumour" = wilcox.test(
        x = bpscore_primary_tumour_vs_benign[Prostate_Type_Combo == "Benign+Benign", 1 - BPscore],
        y = bpscore_primary_tumour_vs_benign[Prostate_Type_Combo == "Benign+Malignant", 1 - BPscore],
        paired = FALSE
    ),
    "primary_tumour-only_benign-vs-tumour" = wilcox.test(
        x = bpscore_primary_tumour_vs_benign[Prostate_Type_Combo == "Malignant+Malignant", 1 - BPscore],
        y = bpscore_primary_tumour_vs_benign[Prostate_Type_Combo == "Benign+Malignant", 1 - BPscore],
        paired = FALSE
    ),
    "primary_benign-only_tumour-only" = wilcox.test(
        x = bpscore_primary_tumour_vs_benign[Prostate_Type_Combo == "Benign+Benign", 1 - BPscore],
        y = bpscore_primary_tumour_vs_benign[Prostate_Type_Combo == "Malignant+Malignant", 1 - BPscore],
        paired = FALSE
    )
)
...
mat <- as.matrix(
    dcast(bpscore[w == MAX_WINDOW], s1 ~ s2, value.var = "BPscore", fill = 0),
    rownames = "s1"
)
...
gg_bpscore_primary_tumour <- (
    ...
    + geom_point(
        aes(x = w, y = 1 - BPscore, colour = w),
        position = position_jitter(width = 0.2, height = 0)
    )
    + geom_boxplot(
        aes(x = w, y = 1 - BPscore, group = w),
        alpha = 0.5,
        outlier.shape = NA,
        width = 0.5
    )
...
savefig(gg_bpscore_primary_tumour, file.path(PLOT_DIR, "bp-score.tumour"))
...
savefig(gg_bpscore_primary_benign, file.path(PLOT_DIR, "bp-score.benign"))
...
savefig(gg_bpscore_primary_tumour_vs_benign, file.path(PLOT_DIR, "bp-score.tumour-vs-benign"))
...
gg_bpscore_primary_tumour_vs_benign <- (
    ...
    + geom_ribbon(
        data = bpscore[
            ...
            & (Source_1 == "Primary" & Source_2 == "Primary")
            & (Source_1 == "Primary" & Source_2 == "Primary"),
            .(Mean = mean(1 - BPscore), SD = sd(1 - BPscore)),
...
savefig(gg_bpscore_primary_tumour_vs_benign, file.path(PLOT_DIR, "bp-score.tumour-vs-benign.stdev"))

...
BAM_DIR <- file.path(
    "..", "..", "data", "Processed",
    "2019-05-03_PCa-H3K27ac-peaks", "BAMs"
)
PEAK_DIR <- file.path(
    "..", "..", "data", "Processed",
...
RES_DIR <- file.path(
    "..", "..", "results", "2020-06-12_sv-disruption-acetylation"
)
ACETYL_DIR <- file.path(RES_DIR, "Acetylation")
TEST_DIR <- file.path(ACETYL_DIR, "T2E")
...
local_comp <- comp_dt[(
    (chr == local_region$chr)
    & (start <= local_region$end)
    & (end >= local_region$start)
)]

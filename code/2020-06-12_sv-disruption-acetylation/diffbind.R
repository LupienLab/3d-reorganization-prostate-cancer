...
BAM_DIR <- file.path(
    "..", "..", "data", "Processed", "2019-05-03_PCa-H3K27ac-peaks", "BAMs"
)
PEAK_DIR <- file.path(
    "..", "..", "data", "Processed", "2019-05-03_PCa-H3K27ac-peaks", "Peaks"
)
...
metadata[, `:=`(
    bamReads = file.path(
        BAM_DIR,
        paste0(gsub("PCa", "Pca", SampleID), "_H3K27ac.sorted.dedup.bam")
    ),
    bamControl = file.path(
        BAM_DIR,
        paste0(gsub("PCa", "Pca", SampleID), "_input.sorted.dedup.bam")
    ),
    Peaks = file.path(
        PEAK_DIR,
        paste0(SampleID, "_peaks.filtered.sorted.merged.narrowPeak")
    ),
    PeakCaller = "narrow"
)]
...
tads <- fread(
    file.path(
        "..", "..", "results", "2020-02-19_sv-disruption-TADs",
...
for (i in 1:length(all_comparisons)) {
    ...
    for (j in 1:tests[, .N]) {
        ...
        if (identical(test_mut, comp_mut) && identical(test_nonmut, comp_nonmut)) {
            all_comparisons[[i]]$test_IDs <- c(
                all_comparisons[[i]]$test_IDs,
                tests[j, test_ID]
            )
        }
    }
...
reduced_comparisons_no_lowqual <- lapply(
    ...
    function(comp) {
        ...
        comp_idx_mut <- which(sapply(
            all_comparisons_no_lowqual,
            function(all_comp) identical(all_comp$mut, comp$mut)
        ))
        comp_idx_nonmut <- which(sapply(
            all_comparisons_no_lowqual,
            function(all_comp) identical(all_comp$nonmut, comp$nonmut)
        ))
        comp_idx <- sort(unique(intersect(comp_idx_mut, comp_idx_nonmut)))
        combined_tests <- sort(unique(unlist(sapply(
            comp_idx, function(i) all_comparisons_no_lowqual[[i]]$test_IDs)
            )))
        return(list(
            mut = comp$mut,
            nonmut = comp$nonmut,
...
rm(
    ...
    all_comparisons_no_lowqual,
    degenerate_comparisons,
    reduced_comparisons_no_lowqual
)
...
for (i in 1:length(all_comparisons)) {
    ...
    local_comp_regions$FDR <- p.adjust(
        local_comp_regions$`p-value`,
        method = "fdr"
    )

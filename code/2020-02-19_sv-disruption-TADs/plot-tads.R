...
RES_DIR <- file.path("..", "..", "results", "2020-02-19_sv-disruption")
...
altered_tads <- fread(
    file.path(RES_DIR, "sv-disruption-tests.TADs.tsv"),
    sep = "\t",
    header = TRUE
)
...
fwrite(
    altered_tads_count,
    file.path(RES_DIR, "sv-disruption-tests.TADs.counts.tsv"),
    sep = "\t",
    col.names = TRUE
)

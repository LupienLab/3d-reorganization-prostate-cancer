...
RES_DIR <- file.path("..", "..", "results", "2020-06-18_sv-disruption-expression")
...
saveRDS(
    ...
    file.path(
        RES_DIR,
        "sleuth",
        paste0("test_", ARGS$test_ID, ".sleuth-object.rds")
    )
)
...
test_genes <- so_genes[
    chr == test_tad$chr & start <= test_tad$end & end >= test_tad$start
]
test_transcripts <- so_transcripts[
    chr == test_tad$chr & start <= test_tad$end & end >= test_tad$start
]
...
fwrite(
    ...
    file.path(
        RES_DIR,
        "sleuth",
        paste0("test_", ARGS$test_ID, ".genes.tested.tsv")
    ),
    sep = "\t",
    col.names = TRUE
)
fwrite(
    ...
    file.path(
        RES_DIR,
        "sleuth",
        paste0("test_", ARGS$test_ID, ".transcripts.tested.tsv")
    ),
    sep = "\t",
    col.names = TRUE
)

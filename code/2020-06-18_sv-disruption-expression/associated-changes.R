...
sv_disruption <- fread(
    file.path(RES_DIR, "summary-sv-disruption-impact.tsv")
)
...
svtype_alteration_contingency_mat <- svtype_alteration_contingency_mat[,
    .SD,
    .SDcols = c("altered_Exprs", "any_BND", "only_same")
]
...
ggsave(
    file.path(PLOT_DIR, "sv-disruption-chromosomal.png"),
    height = 12,
    width = 20,
    units = "cm"
)
ggsave(
    file.path(PLOT_DIR, "sv-disruption-chromosomal.pdf"),
    height = 12,
    width = 20,
    units = "cm"
)
...
fwrite(
    svtype_alteration_contingency_mat,
    file.path(RES_DIR, "sv-disruption-tests.exprs.sv-type.tsv"),
    sep = "\t"
)

fwrite(
    tad_exprs_contingency_mat,
    file.path(RES_DIR, "sv-disruption-tests.exprs-TADs.tsv"),
    sep = "\t"
)

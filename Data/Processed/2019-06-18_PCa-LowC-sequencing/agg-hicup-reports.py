...
def main(cfg, outdir):
    ...
    truncation = pd.read_csv(
        path.join('..', '..', 'Raw', '190509_A00827_0007_AHKYH3DSXX',
                  'Reports', 'hicup_truncater_summary.tsv'),
        sep='\t', index_col=False
    )
    mapping = pd.read_csv(
    ...
    for i, r in agg_data.iterrows():
        ...
        r['Percentage_Mapped'] = np.round(100 * (r['Paired_Read_1'] + r['Paired_Read_2']) / (
            r['Total_Reads_1'] + r['Total_Reads_2']), decimals=2)
        r['Percentage_Valid'] = np.round(
            100 * r['Valid_Pairs'] / (r['Valid_Pairs'] + r['Invalid_Pairs']), decimals=2)
        r['Percentage_Uniques'] = np.round(
            100 * r['Deduplication_Read_Pairs_Uniques'] / r['Valid_Pairs'], decimals=2)
        r['Percentage_Unique_Trans'] = np.round(
            100 * r['Deduplication_Trans_Uniques'] / r['Deduplication_Read_Pairs_Uniques'], decimals=2)
        r['Percentage_Ditags_Passed_Through_HiCUP'] = np.round(
            200 * r['Deduplication_Read_Pairs_Uniques'] / (r['Total_Reads_1'] + r['Total_Reads_2']), decimals=2)
